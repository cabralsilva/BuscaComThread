...
public class IndexBean implements Serializable {
	...
	public void buscarListas() throws SQLException {
		...
		sizeListaPropria = lstSelected.size();
	...
	public String montarMsgSucesso() {
		...
		if (this.tituloPesquisa != null
				&& (this.tituloPesquisa.getTextoPesquisa() != "" || this.tituloPesquisa.getIdiomaTitulo() != null
						|| this.tituloPesquisa.getCampo() != null || this.tituloPesquisa.getTipoTitulo() != null)) {
			...
			if (this.tituloPesquisa.getIdiomaTitulo() != null) {
				retorno = String.valueOf(retorno) + " no idioma '"
						+ this.tituloPesquisa.getIdiomaTitulo().getNomeIdioma() + "'";
			}
			retorno = this.tituloPesquisa.getCampo() != null
					? String.valueOf(retorno) + " no campo '" + this.tituloPesquisa.getCampo().getDescricao() + "'"
					: String.valueOf(retorno) + " em todos os campos";
...
//			 System.out.println("SESSÃO EXPIROU");
			...
			if (tituloPesquisa.getTextoPesquisa().length() < 3) {
				...
				try {
					tituloReturns = new LazyDataModel<Titulo>() {
						...
						public List<Titulo> load(int first, int pageSize, String sortField, SortOrder sortOrder,
							...
							if (tituloPesquisa.getTextoPesquisa().length() < 3) {
								messages.error("Digite um informação maior no campo texto (Mínimo 3 caracteres)!");
								// RequestContext.getCurrentInstance().update(Arrays.asList(":frm:msgs"));
								sessao.setAttribute("indicePrimeiro", 1);
								sessao.setAttribute("indiceUltimo", 1);
								sessao.setAttribute("paginaAtual", 0);
								totalRegistros = 0;
								setRowCount(0);
								return null;
							} else {
								...
								System.out.println("Tempo Pesquisa Completa: "
										+ (System.currentTimeMillis() - tempoInicio) + " ms");
								sessao.setAttribute("lstLazyData", lst);
								return lst;
					...
					// messages.info(montarPaginacaoString(sessao));
				...
				tituloReturns = new LazyDataModel<Titulo>() {
					...
					public List<Titulo> load(int first, int pageSize, String sortField, SortOrder sortOrder,
						...
						if (tituloPesquisa.getTextoPesquisa().length() < 3) {
							messages.error("Digite um informação maior no campo texto (Mínimo 3 caracteres)!");
							sessao.setAttribute("indicePrimeiro", 0);
							sessao.setAttribute("indiceUltimo", 0);
							sessao.setAttribute("paginaAtual", 0);
							totalRegistros = 0;
							// setRowCount(0);
							RequestContext.getCurrentInstance().update(Arrays.asList(":frm:msgs"));
							...
							if (sessao.getAttribute("indicePrimeiro") != null) {
								sessao.setAttribute("paginaAtual",
										(int) sessao.getAttribute("indicePrimeiro") / pageSize);
	...
	public void onRowSelectCheckbox(SelectEvent event) {
		...
		for (Titulo t : lstSelected) {
			...
			for (Titulo s : lstSessao) {
				if (t.getCodigo().equals(s.getCodigo()))
					existe = true;
			}
			if (!existe)
				lstSessao.add(t);
		...
		exibindo();
		sizeListaPropria++;
		RequestContext.getCurrentInstance().update("frm:lblTotalMinhaLista");
	}
	...
	public void onRowUnSelectCheckbox(UnselectEvent event) {
		...
		lstSelected.clear();
		lstSessao = (Set<Titulo>) sessao.getAttribute("listaPropria");
		lstSessao.remove(event.getObject());
		Set<Titulo> lstSessaoNova = new HashSet<>();
		for (Titulo t : lstSessao) {
			if (!t.getCodigo().equals(((Titulo) event.getObject()).getCodigo())) {
				lstSessaoNova.add(t);
			}
		}
		lstSessao = lstSessaoNova;
	...
	public void onAllSelect(AjaxBehaviorEvent a) {
		...
		if (lstSelected.size() == 0) {
			...
			for (Titulo t : lstSessao) {
				boolean existe = false;
				for(Titulo lazy : ((List<Titulo>)sessao.getAttribute("lstLazyData"))){
					if (t.getCodigo().equals(lazy.getCodigo())) {
						existe = true;
						break;
					}
				}
				if (!existe) lstSessaoNova.add(t);
			}
			lstSessao = lstSessaoNova;
			sessao.setAttribute("listaPropria", lstSessao);
		} else {
		...
		sizeListaPropria = ((Set<Titulo>) sessao.getAttribute("listaPropria")).size();
		RequestContext.getCurrentInstance().update("frm:lblTotalMinhaLista");
	}
	...
	public int getSizeListaPropria() {
		return sizeListaPropria;
	}
